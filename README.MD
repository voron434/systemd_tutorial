...
- Проверьте. Сымитируйте поломку программы - резко прервите работу файла с помощью команды `systemctl kill my_http_server`
- Проверьте сайт. Убедитесь, что не работает.

Сервер не перезагружался, он всё еще работает, мы можем "гулять" по директориям внутри сервера, но сайт не работает.

Нужно сделать так, чтобы сайт продолжал работать даже после сбоя файла или чего-то еще, что его остановило.

Скажите SystemD, чтобы при остановке работы файла, чтобы он перезапускался.
