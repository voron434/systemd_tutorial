...
2. Проверьте открывается ли сайт. В браузере ip-адресу вашего сервера на порте 8000.

![image](git_assets/landing_off.png)

Браузер не получил от сервера код сайта.
Оказалось, что недостаточно просто загрузить файл на сервер.

На запрос браузера сервер должен отвечать html-кодом, т.е. в нашем случае исходным кодом сайта `index.html`.

Этим занимается HTTP-сервер.
