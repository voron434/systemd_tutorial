...
Мы загрузили этот `index.html` на сервер, но сайт не работает. Дело в том, что сервер не запущен.
...
3. Сайт запущен! Можно выходить из терминала.

4. Просто закройте окошко терминала или введите `exit` и затем нажмите `Enter`.

5. Увидьте, что сайт перестал работать.

Основная причина этому - ограниченность ресурсов сервера, он не может держать бесконечное количество активных подключений.

Поэтому он отключает все неактивные подключения.

Мы, конечно, можем сделать так, чтобы время нашего подключения было неограниченным, но это небезопасное решение.
...
Специальная программа **SystemD**, которая встроена в Операционную Систему нашего сервера, запустит нашу программу в режиме демона.
...
2. Создайте специальный файл-инструкцию с названием `my_http_server.service` и с таким содержанием:
    ...
    ```
    > Если коротко, то там написано следующее: *"При запуске файла надо запустить команду python -m http.server 8000 "*
...
4. Наберите `systemctl status my_http_server`
    ...
    Там написано:
    - 3-я строка сверху: `Active: inactive (dead)` - говорит о том, что наш файл, который мы создали неактивен/"мёртв"
    - 3-я строка снизу (выделено красным): `/etc/systemd/system/my_http_server.service:2: Executable "python" not found in path "/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"` - говорит о том, что конкретно случилось? какая ошибка произошла?
...
7. В этот раз терминал ничем не ответил. Проверьте состояние файла-инструкции с помощью команды из пункта №4: `systemctl status my_http_server`
    ...
    В этот раз написано:
    - 2-я строка сверху: `●my_http_server.service`. Кружок возле названия нашего файла стал зелёным, а в прошлый раз был черный.
    - 4-я строка сверху: `Active: active (running)`. Слова `active (running)` означают "активный (запущен)". Как и кружок они стали зелеными, а в прошлый раз они были красными.
...
- Пройдите, что сайт действительно не работает:
...
4. Перезагрузите сервер с помощью команды `reboot now`

5. Убедитесь, что сайт продолжает работать.

### 4. Автозапуск в случае сбоя

Поздравляю! Сайт продолжает работать даже если вы закроете терминал, прервете соединение с сервером или даже после перезагрузки сервера.

Однако программы по разным причинам могут "ломаться" и переставать работать.

Так и, например, `http.sever` или сам `python-интерпретатор` может поломаться или просто произойдет какой-то сбой, и в итоге наш файл-инструкция `my_http_server` остановится.

В итоге сайт просто "упадет".


- Убедитесь в этом. Резко прервите работу файла с помощью команды `systemctl kill my_http_server`
- Проверьте сайт. Видите? Не работает.
...
5. Проверьте состояние работы файла с помощью команды `systemctl status my_http_server`: 

    ![image](git_assets/after_daemon_reload.png)

    - Файл запустился. Сайт работает.
    
6.  Резко прервите работу файла с помощью команды `systemctl kill my_http_server`

7. Ещё раз проверим состояние работы файла `systemctl status my_http_server`

    ![image](git_assets/after_kill2.png)

    - Супер! Всё работает - и файл, и сайт работают.

Поздравляю! Теперь сайт будет работать и после закрытия терминала, и после перезагрузки, и даже если поломается программа в файле.
...
Иногда нужно посмотреть историю только по определенному файлу-инструкции.
...
- Запустите команду `journalctl -f -u my_http_server`.
    - Будете видеть историю по файлу в режиме реального времени (на экране будут добавляться новые записи).
