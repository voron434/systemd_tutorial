...
2. Загрузите исходный код [сайта](https://github.com/su1tanb3k/systemd_tutorial/blob/master/index.html) в корневую директорию сервера.

3. Запустите сервер с помощью команды `python3 -m http.server 8000`.

4. По ip-адресу вашего сервера на порте 8000 появится сайт:

![image](git_assets/landing.png)

### 2. Отключитесь от сервера

Сайт запущен. Можно выходить из терминала.
...
Мы, конечно, можем сделать так, чтобы время подключения было неограниченным, но это небезопасное решение.
...
Специальная программа **SystemD**, которая встроена в ОС нашего сервера, поможет запустить нашу программу в режиме демона.
