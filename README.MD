...
Загрузите его на сервер в корневую директорию с помощью команды:

`wget https://github.com/su1tanb3k/systemd_tutorial/blob/master/index.html`

Проверьте открывается ли сайт. В браузере пройдите на порт 8000 на ip-адресе вашего сервера (Пример: `84.38.180.108:8000`).

![image](git_assets/landing_off.png)

Сайт не работает, потому что сервер не отвечает браузеру. Чтобы сервер отвечал браузеру нужно запустить HTTP-сервер.

### 1. Запустите HTTP-сервер

Запустите сервер. Введите в терминале команду `python3 -m http.server 8000`
...
Увидьте, что сайт перестал работать.

Основная причина - это ограниченность ресурсов сервера. Поэтому ему приходится отключать все неактивные подключения.

Он не может держать бесконечное количество активных подключений.
...
Перейдите в эту директорию: `/etc/systemd/system`

Создайте специальный файл-инструкцию с названием `my_http_server.service` и с таким содержанием:
...
Наберите `systemctl status my_http_server`
...
Повторите запуск с помощью команды из пункта №3: `systemctl start my_http_server`.
>В этот раз терминал ничем не ответил

Проверьте состояние файла-инструкции с помощью команды из пункта №4: `systemctl status my_http_server`
...
Пройдите в браузере по адресу `ip-сервера:8000`. Видите, что сайт работает? Успех!

А точно успех? Убедитесь в этом.
...
Пройдите в браузере по адресу `ip-сервера:8000`.
...
Убедитесь, что сайт продолжает работать.
...
Сымитируйте поломку программы. "Резко" остановите работу файла с помощью команды:
...
Запускаем команду `systemctl daemon-reload`

Повторите запуск `systemctl start my_http_server`.

Резко прервите работу файла с помощью команды `systemctl kill my_http_server`
